...
│                    (Порт 5149)                              │
│                                                             │
│  "Польовий" сервіс для одного рою дронів                   │
│                                                             │
│  • Управління дронами (створення, оновлення)               │
│  • Управління Hive (групи дронів)                          │
│  • Топології (mesh, star, dual-star)                       │
│  • Mesh-маршрутизація команд                               │
│  • Аналіз зв'язності рою                                   │
│  • Емуляція деградації зв'язків                            │
│  • Телеметрія до Communication Control                    │
└───────────────────────┬─────────────────────────────────────┘
                        │
                        │ Redis Pub/Sub
                        │
...
│                 База даних для телеметрії                    │
└─────────────────────────────────────────────────────────────┘
                        │
                        │
┌───────────────────────▼─────────────────────────────────────┐
│                 Map Client (Порт 3000)                      │
│                 React-додаток для візуалізації               │
└─────────────────────────────────────────────────────────────┘
```
...
### 3. **Map Client** (Порт 3000)
**Роль:** Візуалізація дронів на карті

**Технології:**
- React + Vite
- Відображення позицій дронів
- Взаємодія з Communication Control API

---

### 4. **Redis** (Порт 6379)
**Роль:** Message Bus для телеметрії

**Використання:**
- Публікація телеметрії від Communication Control
- Може використовуватися для подальшої обробки
...
**Використання:**
- Зберігання телеметрії (якщо потрібно)
- Може використовуватися для історії команд
...
```
HiveMind API (кожні 5 сек для кожного Hive)
   ↓
...
Communication Control:
   ...
   • Автоматично запитує інформацію про зв'язність
   • Логує детальну інформацію про топологію
```
...
- **.NET 8** - всі API сервіси (Communication Control, HiveMind)
- **React + Vite** - Map Client (frontend)
- **PostgreSQL** - база даних (опціонально для зберігання)
- **Redis** - message bus для телеметрії
- **Docker Compose** - оркестрація всіх контейнерів
